...
struct LEFT_EDGE  L_edge[AMOUNT];       //左边线结构体
struct RIGHT_EDGE R_edge[AMOUNT];       //右边线结构体
...
{
    tft180_show_gray_image(0, 0, image[0], LIN, ROW, LIN, ROW, 0);//显示二值化后的图像
    for(int i = 0; i < AMOUNT; i++)//边线打印
    {
        tft180_draw_point((uint16)L_edge[i].col, (uint16)L_edge[i].row, RGB565_RED);//红色显示左边线
        tft180_draw_point((uint16)R_edge[i].col, (uint16)R_edge[i].row, RGB565_BLUE);//蓝色显示右边线
    }
    tft180_show_string(0, 80, "time:");
    tft180_show_float (40, 80, (float)tim/1000, 2, 3);//显示扫线所需时间，单位：ms
